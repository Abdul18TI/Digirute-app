...
class FasilitasUmumRWController extends Controller
{
    public function index()
    {
        // $fasilitas = Fasilitas_umum::where('status_fasilitas', 1)->get();
        // dd($fasilitas);
        return view('RW.fasilitas.fasilitas_umum', [
    ...
    public function create()
    {
        ...
        return view('RW.fasilitas.tambah_fasilitas', [
    ...
    public function edit(Fasilitas_umum $fasilitasrw)
    {
        return view('RW.fasilitas.edit_fasilitas', [
            'fasilitas' => $fasilitasrw,
            'kategori_fasilitas' => Kategori_fasilitas_umum::all(),
            'title' => 'edit-fasilitas'
        ]);
    }

    public function update(Request $request, Fasilitas_umum $fasilitasrw)
    {
        $validatedData = $request->validate([
            'fasilitas_umum' => 'required',
            'kategori_fasilitas_umum' => 'required',
        ...
        Fasilitas_umum::where('id_fasilitas_umum', $fasilitasrw->id_fasilitas_umum)
            ->update($validatedData);
        return redirect()->route('rw.fasilitasrw.index')->with('success', 'Data berhasil diubah!');
    }

    public function destroy(Fasilitas_umum $fasilitasrw)
    {
        // $fasilitas->delete();
        ...
        try {
            $fasilitasrw->delete();
            if ($fasilitasrw->foto_fasilitas) {
                Storage::delete($fasilitasrw->foto_fasilitas);
            }
            return redirect()->route('rw.fasilitasrw.index')
    ...
    public function show($id)
    {
        ...
        return view('RW.fasilitas.detail_fasilitas', [
            'fasilitas' => $fasilitas,
