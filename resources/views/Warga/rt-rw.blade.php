...
@section('title')Profile RT {{ auth()->user()->rt_rel->no_rt }} RW {{auth()->user()->rt_rel->rw_rel->no_rw }}
 {{ $title }}
@endsection
	    ...
	    <div class="row justify-content-center">
	        <div class="col-md-6 col-lg-6 col-xl-4 box-col-6">
	            <div class="card custom-card shadow shadow-showcase">
	                <div class="card-header"><img class="img-fluid" src="https://source.unsplash.com/2560x1600?indonesia-nature" alt="" /></div>
	                ...
	                <div class="text-center profile-details">
	                    <a href="#"><h4 >{{ $rt->ketua_rt}}</h3></a>
	                    <h6 class="mb-0">RT {{ $rt->no_rt}}</h6>
	                    <h6 class="f-w-400">{{ date('Y', strtotime($rt->tgl_awal_jabatan_rt) )}} -  {{ date('Y', strtotime($rt->tgl_akhir_jabatan_rt) )}}</h6>
	                </div>
	        ...
	        <div class="col-md-6 col-lg-6 col-xl-4 box-col-6">
	            <div class="card custom-card shadow shadow-showcase">
	                <div class="card-header"><img class="img-fluid" src="https://source.unsplash.com/random/2560x1600?indonesia-nature" alt="" /></div>
					<div class="card-profile"><img class="rounded-circle" src="{{asset('assets/images/avtar/'. $rt->identitas_rt->foto_warga)}}" alt="" /></div>
	                <ul class="card-social">
						<li>
	                ...
	                <div class="text-center profile-details">
	                    <a href="#"><h4 >{{ $rw->ketua_rw}}</h3></a>
	                    <h6 class="mb-0">RW {{ $rw->no_rw}}</h6>
	                    <h6 class="f-w-400">{{ date('Y', strtotime($rw->tgl_awal_jabatan_rw) )}} -  {{ date('Y', strtotime($rw->tgl_akhir_jabatan_rw) )}}</h6>
	                </div>
	                <div class="card-footer row">
	                    <div class="col-12 col-sm-12 mt-2">
	                        <h3 class="txt-primary">Kontak</h3>
